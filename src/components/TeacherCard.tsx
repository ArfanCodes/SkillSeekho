...
import { Star, MapPin, Heart, BadgeCheck, ShieldCheck, MessageCircle, Calendar } from 'lucide-react';
import type { Mentor } from '../types';

interface Props { mentor: Mentor; index?: number; }

export default function TeacherCard({ mentor, index = 0 }: Props) {
  const { name, avatar, avatarColor, photo, skill, distance, rating, reviews, vouches,
          pricePerSession, currency, verified, location: loc, tags, badges = [], bio } = mentor;
  ...
  return (
    <motion.div
      ...
      <div className="teacher-card__image">
        {photo
          ? <img src={photo} alt={name} className="teacher-card__photo" loading="lazy" />
          : <div className="teacher-card__avatar-fallback" style={{ backgroundColor: avatarColor }}>{avatar}</div>
        }
        {verified && (
          <div className="teacher-card__verified-badge">
            <ShieldCheck size={12} /><span>Verified</span>
          </div>
        )}
      ...
      <div className="teacher-card__details">
        <div className="teacher-card__header">
          <div className="teacher-card__name-row">
            ...
            {verified && <BadgeCheck size={16} fill="#22C55E" color="white" className="flex-shrink-0" />}
          </div>
        ...
        {badges.length > 0 && (
          <div className="teacher-card__badges">
            {badges.map((badge) => <span key={badge} className="teacher-card__badge">{badge}</span>)}
          </div>
        )}
        ...
        <div className="teacher-card__actions">
          ...
          <motion.button whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.9 }}
            className="teacher-card__btn-icon" aria-label="Save teacher">
            <Heart size={16} />
          </motion.button>
        </div>
      </div>
