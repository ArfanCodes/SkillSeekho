-- ─────────────────────────────────────────────
-- Make typo tolerance actually work.
--
-- 018/019 used similarity(lower(title), tok), which compares the WHOLE title
-- string to one query word — a single typo'd word gets diluted by the other
-- words in the title and falls under threshold (e.g. "biriyani" vs the title
-- "Hyderabadi Dum Biryani" scored too low and matched nothing).
--
-- word_similarity(tok, text) instead scores the query word against the best
-- matching word *inside* the text, so "biriyani" matches the "biryani" word.
-- Applied across title, description and tags so a near-miss never fails.
-- ─────────────────────────────────────────────
          ...
          WHERE tok NOT IN (
            ...
            AND (
              ...
              (length(tok) >= 3 AND (
                    ...
                    -- typo / word-variant tolerance for longer words: match the
                    -- query word against the closest WORD inside each field.
                    OR (length(tok) >= 4 AND (
                          word_similarity(tok, lower(s.title)) > 0.6
                          OR word_similarity(tok, lower(coalesce(s.description, ''))) > 0.6
                          OR EXISTS (
                            SELECT 1 FROM unnest(s.tags) t
                            WHERE word_similarity(tok, lower(t)) > 0.6
                          )
                       ))
              ))
